...
def get_args_parser():
    parser = argparse.ArgumentParser('D-iGPT pre-training', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='DiGPT_vit_base', type=str, metavar='MODEL',
...
def main(args):
    ...
    # define the model
    model = models_digpt.__dict__[args.model]()

    state_dict = torch.load(args.clip_path, map_location="cpu")
    teacher = clip.VisionTransformer(width=state_dict["visual.conv1.weight"].shape[0],
        ...
        if len(ckpt) > 0:
            for i in range(800):
                if "checkpoint-{}.pth".format(str(i)) in ckpt:
                    args.resume = args.output_dir + "/checkpoint-{}.pth".format(str(i))
                    print(args.resume)
